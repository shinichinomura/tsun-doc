...
class ResultSet extends Component {
  select_item(event, asin, title, url, thumbnail_url) {
    const success_action = this.props.actions.select_item

    request
      .post('/user_amazon_books')
      .send({asin, title, url, thumbnail_url, authenticity_token: document.getElementsByName('csrf-token')[0]['content']})
      ...
      <div>
        ...
        { this.props.resultSet.length > 0 &&
          ...
          { this.props.resultSet.map(item =>
            <li data-asin={item.asin} key={item.asin} onClick={(event) => this.select_item(event, item.asin, item.title, item.url, item.small_image_url)}>
